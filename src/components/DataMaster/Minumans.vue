<template>
  <v-main class="list">
    <h3 class="font-weight-medium"
       style="text-align: center; font-style: italic;  font-size:72px; color:green;" font-weight-medium mb-5>Daftar Minuman</h3>
    ...
    <v-card>
      <v-card-title>
        ...
        <v-btn color="success" dark @click="dialog = true">Tambah</v-btn>
            ...
            <v-data-table :headers="headers" :items="drinks" :search="search">
        ...
        <v-dialog v-model="dialogConfirm" persistent max-width="400px">
            <v-card>
                ...
                <v-card-text>
                    Anda yakin ingin menghapus minuman ini?
                </v-card-text>
                <v-card-action>
                    <v-spacer></v-spacer>
...
export default {
    ...
    methods: {
        ...
        //Simpan data Drink
        save() {
            this.drink.append('name', this.form.name);
            this.drink.append('stok', this.form.stok);
            this.drink.append('harga', this.form.harga);

            var url = this.$api + '/drink/'
            this.load = true;
            this.$http.post(url, this.drink, {
                headers: {
                    'Authorization' : 'Bearer ' + localStorage.getItem('token'),
                }
            }).then(response => {
        ...
        update(){
            ...
            var url = this.$api + '/drink/' + this.editId;
            this.load = true;
        ...
        // Hapus data Drink
        deleteData(){
            var url = this.$api + '/drink/' + this.deleteId;
            this.load = true;
